 ...
 * Step 3: Memory Write Operations (Complete) - FIXED
...
/* Memory Write Structures (Step 3) - UPDATED */
...
struct physical_mem_write {
    ...
    unsigned long length;
    unsigned char *user_buffer;
    int method;  /* 0=auto, 1=ioremap, 2=direct, 3=kmap */
};
...
/* ========================================================================
 * Step 3: Memory Write Operations - FIXED with multiple methods
 * ======================================================================== */
...
/*
 * Write to physical memory with method selection
 * method: 0=auto, 1=ioremap, 2=direct map, 3=kmap
 */
void write_physical_mem_method(unsigned long phys_addr, const unsigned char *data, 
                                size_t size, int method) {
    const char *method_names[] = {"auto", "ioremap", "direct map (__va)", "kmap (PFN)"};
    ...
    printf("[*] Writing %zu bytes to physical 0x%lx (method: %s)\n", 
           size, phys_addr, method_names[method % 4]);
    
    if (ioctl(fd, IOCTL_WRITE_PHYSICAL_MEM, &req) < 0) {
        perror("[-] write_physical_mem failed");
        printf("[!] Try a different method:\n");
        printf("    - write_phys_direct: Uses kernel direct map (__va) - best for RAM\n");
        printf("    - write_phys_pfn: Uses kmap via PFN - works for managed pages\n");
        printf("    - write_phys_ioremap: Uses ioremap - for MMIO, usually fails for RAM\n");
        return;
    }
    printf("[+] Write successful\n");
}

/* Auto method - tries all methods */
void write_physical_mem(unsigned long phys_addr, const unsigned char *data, size_t size) {
    write_physical_mem_method(phys_addr, data, size, 0);  /* Auto */
}
...
void write_physical_mem_hex_method(unsigned long phys_addr, const char *hex_data, int method) {
    unsigned char data[512];
    int len = parse_hex_pattern(hex_data, data, sizeof(data));
    if (len < 0) {
        printf("[-] Invalid hex data\n");
    ...
    write_physical_mem_method(phys_addr, data, len, method);
}

/* Direct map method - best for RAM addresses */
void write_physical_direct(unsigned long phys_addr, const unsigned char *data, size_t size) {
    struct physical_mem_write req = {
        .phys_addr = phys_addr,
        .length = size,
        .user_buffer = (unsigned char *)data,
        .method = 0  /* Will use IOCTL_WRITE_PHYSICAL_DIRECT */
    };
    
    printf("[*] Writing %zu bytes to physical 0x%lx (direct map via __va)\n", size, phys_addr);
    
    if (ioctl(fd, IOCTL_WRITE_PHYSICAL_DIRECT, &req) < 0) {
        perror("[-] write_physical_direct failed");
        return;
    }
    printf("[+] Direct write successful\n");
}

void write_physical_direct_hex(unsigned long phys_addr, const char *hex_data) {
    unsigned char data[512];
    int len = parse_hex_pattern(hex_data, data, sizeof(data));
    if (len < 0) {
        printf("[-] Invalid hex data\n");
    ...
    write_physical_direct(phys_addr, data, len);
}

/* PFN/kmap method */
void write_physical_pfn(unsigned long phys_addr, const unsigned char *data, size_t size) {
    struct physical_mem_write req = {
        .phys_addr = phys_addr,
        .length = size,
        .user_buffer = (unsigned char *)data,
    ...
    printf("[*] Writing %zu bytes to physical 0x%lx (via PFN/kmap)\n", size, phys_addr);
    
    if (ioctl(fd, IOCTL_WRITE_PHYSICAL_PFN, &req) < 0) {
        perror("[-] write_physical_pfn failed");
        return;
    }
    printf("[+] PFN write successful\n");
}

void write_physical_pfn_hex(unsigned long phys_addr, const char *hex_data) {
    unsigned char data[512];
    int len = parse_hex_pattern(hex_data, data, sizeof(data));
    if (len < 0) {
        printf("[-] Invalid hex data\n");
    ...
    write_physical_pfn(phys_addr, data, len);
}
...
void print_help(void) {
    ...
    printf("║                           v2.1 - Fixed Physical Writes                   ║\n");
    printf("╚══════════════════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("  write_phys <addr> <hex>      - Write to physical (auto method)\n");
    printf("  write_phys_direct <addr> <hex> - Write via direct map (__va) - BEST FOR RAM\n");
    printf("  write_phys_pfn <addr> <hex>  - Write via PFN/kmap\n");
    printf("  write_phys_ioremap <addr> <hex> - Write via ioremap (MMIO only)\n");
    ...
    printf("  patch <addr> <orig> <new> <type> - Patch with verification\n\n");
    ...
    printf("PHYSICAL WRITE METHODS EXPLAINED:\n");
    printf("  ┌─────────────────────────────────────────────────────────────────────┐\n");
    printf("  │ Method        │ Best For      │ Notes                               │\n");
    printf("  ├─────────────────────────────────────────────────────────────────────┤\n");
    printf("  │ write_phys_direct │ RAM        │ Uses __va(), fastest, most reliable │\n");
    printf("  │ write_phys_pfn    │ RAM        │ Uses kmap_atomic via PFN           │\n");
    printf("  │ write_phys_ioremap│ MMIO       │ Usually FAILS for RAM addresses    │\n");
    printf("  │ write_phys        │ Auto       │ Tries all methods in order         │\n");
    printf("  └─────────────────────────────────────────────────────────────────────┘\n\n");
    ...
    printf("EXAMPLES:\n");
    printf("  ./probe write_phys_direct 0x101cc7218 deadbeef  # Write to RAM\n");
...
int main(int argc, char *argv[]) {
    ...
    /* Memory write operations - FIXED with multiple methods */
    ...
    else if (strcmp(cmd, "write_phys_direct") == 0 && argc > 3)
        write_physical_direct_hex(strtoul(argv[2], NULL, 0), argv[3]);
    else if (strcmp(cmd, "write_phys_pfn") == 0 && argc > 3)
        write_physical_pfn_hex(strtoul(argv[2], NULL, 0), argv[3]);
    else if (strcmp(cmd, "write_phys_ioremap") == 0 && argc > 3)
        write_physical_mem_hex_method(strtoul(argv[2], NULL, 0), argv[3], 1);
