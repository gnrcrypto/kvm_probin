...
struct kernel_mem_write {
    ...
    unsigned long length;
    unsigned char __user *user_buffer;
    int disable_wp_flag;              /* Disable write protection during write */
};
...
static int dump_page_tables(unsigned long virt_addr, struct page_table_dump *dump)
{
    ...
    if (pud_leaf(*pud)) {
        /* PUD leaf mapping (e.g., 1GB page) */
        dump->physical_addr =
            (pud_val(*pud) & PUD_MASK) | (virt_addr & ~PUD_MASK);
        dump->flags |= 0x01;
        return 0;
    }
    ...
    if (pmd_leaf(*pmd)) {
        /* PMD leaf mapping (e.g., 2MB huge page) */
        dump->physical_addr =
            (pmd_val(*pmd) & PMD_MASK) | (virt_addr & ~PMD_MASK);
        dump->flags |= 0x02;
        return 0;
    }
    ...
    dump->physical_addr =
        (pte_val(*pte) & PAGE_MASK) | (virt_addr & ~PAGE_MASK);
...
static int convert_virt_to_phys(unsigned long virt_addr, struct virt_to_phys_request *req)
{
    ...
    /* Check if it's a vmalloc address - use virt_to_phys */
...
static int convert_hva_to_pfn(unsigned long hva, struct hva_to_pfn_request *req)
{
    ...
    /* For kernel addresses, use simpler methods */
    ...
    /* For user addresses, try get_user_pages if we have an mm context */
...
static long driver_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case IOCTL_WRITE_KERNEL_MEM: {
            ...
            ret = write_kernel_memory(req.kernel_addr, kbuf, req.length, disable_wp);
            kfree(kbuf);
            return ret;
        }
